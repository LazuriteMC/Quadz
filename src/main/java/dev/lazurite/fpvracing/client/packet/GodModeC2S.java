...
public class GodModeC2S {
    ...
    public static void accept(PacketContext context, PacketByteBuf buf) {
        ...
        context.getTaskQueue().execute(() -> {
            CompoundTag tag = hand.getOrCreateSubTag(FPVRacing.MODID);
            ...
            if (hand.getItem() instanceof TransmitterItem) {
                if (bind.getKey().getType().fromTag(tag, bind.getKey().getName()) != null) {
                    List<Entity> entities = ((ServerWorld) player.getEntityWorld()).getEntitiesByType(FPVRacing.QUADCOPTER_ENTITY, EntityPredicates.EXCEPT_SPECTATOR);
