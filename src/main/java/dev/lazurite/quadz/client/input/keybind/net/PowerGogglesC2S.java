...
public class PowerGogglesC2S {
    public static final Identifier PACKET_ID = new Identifier(Quadz.MODID, "power_goggles_c2s");
    ...
    public static void accept(MinecraftServer server, ServerPlayerEntity player, ServerPlayNetworkHandler handler, PacketByteBuf buf, PacketSender sender) {
        ...
        server.execute(() -> {
            if (Quadz.GOGGLES_CONTAINER.maybeGet(hand).isPresent()) {
                Quadz.GOGGLES_CONTAINER.get(hand).setEnabled(enable);
            } else if (Quadz.GOGGLES_CONTAINER.maybeGet(hat).isPresent()) {
                Quadz.GOGGLES_CONTAINER.get(hat).setEnabled(enable);
            }
